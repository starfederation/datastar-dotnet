...
module Program =
    ...
    let main args =
        ...
        app.MapGet(
            "/hello-world",
            Func<IHttpContextAccessor, Task>(fun ctx ->
                task {
                    ...
                    |> Seq.map (fun element ->
                        async {
                            do!
                                ServerSentEventGenerator.PatchElementsAsync(ctx.HttpContext.Response, element)
                                |> Async.AwaitTask
        ...
        app.MapGet(
            ...
            Func<IHttpContextAccessor, Task>(fun ctx ->
                task {
                    do! ServerSentEventGenerator.StartServerEventStreamAsync(ctx.HttpContext.Response)
                    do! ServerSentEventGenerator.ExecuteScriptAsync(ctx.HttpContext.Response, "alert('Hello! from the server 🚀')")
                    return ()
                })
        )
